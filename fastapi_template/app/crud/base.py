...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_multi_where_array(
        ...
        result = query.all()
        # if resource != "" and settings.COMMENTS_API:
        #      for res in result:
        #          if hasattr(res, 'id'):
        #              comments = requests.get(f"{settings.COMMENTS_API}/{resource}/{str(res.id)}")
        #              comment_json = comments.json()
        #              res.comment_count = comment_json['meta']['itemCount']
    ...
    def update(
        ...
        update_data = (
            ...
            if user_id
            else {**update_data, "updated_at": func.now()}
        )
    ...
    def get_date_range_filter(
        ...
        if date_range == "today":
            now = datetime.now()
            start_day = now - timedelta(hours=24)
            end_day = now
